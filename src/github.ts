...
export interface Releaser {
    ...
    deleteRelease(params: { owner: string; repo: string; release_id: number }): Promise<{ data: Release }>;
...
export const upload = async (
    ...
    if (resp.status !== 201) {
        ...
        console.log(`Failed to upload asset ${name} (${maxRetries - 1} retries remaining).`);
...
export const release = async (config: Config, releaser: Releaser, maxRetries: number = 3): Promise<Release> => {
    ...
    try {
        ...
        const prerelease =
            config.input_prerelease !== undefined ? config.input_prerelease : existingRelease.data.prerelease;
